...
router.post('/upload/:entityType/:entityId', upload.single('file'), handleMulterError, async (req: express.Request, res: express.Response) => {
  try {
...
router.post('/upload-multiple/:entityType/:entityId', upload.array('files', 10), handleMulterError, async (req: express.Request, res: express.Response) => {
  try {
    const { entityType, entityId } = req.params;
    const { fieldName } = req.query;
...
router.get('/url/:entityType/:entityId/:fileName', async (req, res) => {
  try {
...
router.get('/list/:entityType/:entityId', async (req, res) => {
  try {
    const { entityType, entityId } = req.params;
    const { fieldName } = req.query;
...
// Delete file
router.delete('/:entityType/:entityId/:fileName', async (req, res) => {
  try {
    const { entityType, entityId, fileName } = req.params;
    const { fieldName } = req.query;
...
router.post('/archive/:entityType/:entityId', async (req, res) => {
  try {
    const { entityType, entityId } = req.params;
...
// Generate proxy file
router.post('/proxy/:entityType/:entityId/:fileName', async (req, res) => {
  try {
    const { entityType, entityId, fileName } = req.params;
...
router.get('/proxy/status/:jobId', async (req, res) => {
  try {
